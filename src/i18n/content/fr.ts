...
export const fr: SiteContent = {
  ...
  home: {
    ...
    couponsSec: {
      ...
      cards: [
        ...
        {
          h: 'Donnez-en un, demandez-en un dont vous avez besoin',
          p: 'Un bon que vous n’utiliserez pas avant sa péremption ? Donnez-le et gagnez du Karma — scanner un ticket ou ajouter un prix en rapporte aussi. Besoin d’un bon pour vos courses ? Choisissez le magasin dans <strong>Demander</strong> et Tallywise vous donne celui qui expire le plus tôt.',
        },
      ],
    },
    shots: {
  ...
  coupons: {
    ...
    how: {
      ...
      cards: [
        ...
        {
          h: '3. Donnez-le, ou demandez-en un',
          p: 'Un bon que vous n’utiliserez pas, c’est sûr ? Touchez l’icône cadeau pour le donner — cela vous rapporte du Karma et il rejoint le pool partagé. Besoin d’un bon vous-même ? Ouvrez <strong>Demander</strong>, choisissez le magasin et le type de bon, et Tallywise vous donne celui qui expire le plus tôt.',
        },
      ],
    },
    fair: {
      h2: 'Le Karma garde le partage équitable.',
      ps: [
        'Chaque bon du pool vient du Karma de quelqu’un d’autre. Donnez-en un, scannez un ticket ou ajoutez un prix, et vous en gagnez — un bon rapporte d’autant plus de Karma qu’il est loin de sa péremption. Dépensez-en un peu pour en demander un : choisissez un magasin et un type de bon, et Tallywise vous donne celui qui expire le plus tôt. À court de Karma, la demande se met en pause jusqu’à ce que vous en regagniez.',
        'Votre solde de Karma est privé — personne d’autre ne le voit — et nous n’associons jamais votre nom ou votre e-mail à un bon que vous donnez ; seules les informations propres au bon (valeur, conditions, dates, code-barres) circulent.',
      ],
    },
    faq: {
      h2: 'Questions fréquentes.',
      items: [
        ...
        { q: 'Un bon qu’on m’a donné fonctionnera-t-il en caisse ?', a: 'En général oui — mais les bons sont émis par l’enseigne, pas par Tallywise. Ils sont généralement à usage unique et limités dans le temps. Nous ne pouvons pas vérifier qu’un bon partagé n’a pas déjà été utilisé ; si un bon a été utilisé ou refusé, demandez-en un autre à la place.' },
        { q: 'Qu’est-ce que le Karma ?', a: 'Le Karma est le score d’usage équitable de Tallywise pour le pool de bons — juste des points, pas une monnaie. Vous le gagnez en donnant un bon, en scannant un ticket ou en ajoutant un prix, et en dépensez un peu à chaque fois que vous demandez un bon dans le pool. Vous seul voyez votre propre solde.' },
  ...
  privacy: {
    ...
    heroSub: 'Dernière mise à jour le 21 août 2026. Tallywise est exploitée par MonkeyLittle. Questions : <a href="mailto:%EMAIL%">%EMAIL%</a>.',
    blocks: [
      ...
      { h: 'Partage de bons', ps: ['Si vous choisissez de donner un bon, ses informations (valeur, conditions, dates de validité, le magasin et le code-barres/numéro nécessaire pour l’utiliser) sont ajoutées à un pool partagé que d’autres utilisateurs Tallywise peuvent demander par magasin et type de bon. C’est un choix que vous faites <strong>bon par bon</strong> — les bons que vous vous contentez d’enregistrer restent privés à votre compte. Nous ne partageons pas votre identité à côté d’un bon mis en pool. Donner ou demander un bon gagne ou dépense du <strong>Karma</strong>, notre score d’usage équitable pour le pool. Un bon que vous donnez quitte votre portefeuille et peut être demandé par quelqu’un d’autre ; vous pouvez le reprendre tant qu’il est dans le pool.'] },
  ...
  terms: {
    ...
    blocks: [
      ...
      { h: '13. Suspension &amp; résiliation', ps: ['Vous pouvez cesser d’utiliser Tallywise à tout moment et supprimer votre compte depuis les Réglages de l’app ou en nous écrivant. Nous pouvons suspendre ou mettre fin à votre accès si vous enfreignez les présentes conditions ou lorsque cela est raisonnablement nécessaire pour protéger le service ou les autres utilisateurs. En cas de suppression, les données de votre compte sont supprimées comme décrit dans notre <a href="%PRIVACY%">politique de confidentialité</a> ; les données de prix anonymisées peuvent être conservées.'] },
      {
        ...
        ps: [
          'Tallywise vous permet d’enregistrer des bons de réduction et, en option, de les donner à, ou de les demander à, d’autres utilisateurs via un pool partagé. Les informations des bons (valeur, conditions, dates de validité et code-barres/numéro) sont lues par un scan sur l’appareil et <strong>peuvent être mal lues</strong> — vérifiez-les par rapport au bon d’origine avant de vous y fier.',
          ...
          'Les bons partagés entre utilisateurs sont fournis <strong>« en l’état » et à vos risques</strong>. Un bon que vous demandez peut avoir déjà été utilisé, avoir expiré ou être refusé en caisse, et nous <strong>ne pouvons ni vérifier ni garantir qu’un bon est valide, non utilisé ou accepté</strong>. Dans toute la mesure permise par la loi, MonkeyLittle n’est pas responsable d’un bon qui ne fonctionne pas, d’un bon partagé par un autre utilisateur, ni d’un litige entre utilisateurs au sujet d’un bon partagé.',
          'Le partage repose sur le <strong>Karma</strong>, le score d’usage équitable de Tallywise pour le pool : donner un bon, scanner un ticket ou ajouter un prix gagne du Karma, et demander un bon en dépense — ainsi le pool ne peut pas être vidé par une poignée de comptes. Nous pouvons fixer, modifier ou appliquer la façon dont le Karma est gagné ou dépensé, et retirer des bons ou restreindre des comptes, pour prévenir les abus du pool.',
        ],
      },
      {
